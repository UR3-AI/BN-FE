...
import type { ChatSessionListResponse, UseChatSessionsQueryParams } from "./useChatSessionsQuery.type";

const fetchChatSessions = async (params: UseChatSessionsQueryParams) => {
  const response = await api.get<ChatSessionListResponse>("/api/v1/chat/sessions", { params });
